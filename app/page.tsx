...
function Filters() {
  return (
    <div className="filters" id="filters">
      ...
      <select id="brand">
        <option>Toate</option>
        {Array.from(new Set(catalogs.map((c:any) => c.brand))).map((b:any) => <option key={b}>{b}</option>)}
      </select>
      <select id="year">
        <option>Toți anii</option>
        {Array.from(new Set(catalogs.map((c:any) => c.year))).sort((a:any,b:any)=>b-a).map((y:any) => <option key={y}>{y}</option>)}
      </select>
      <button onClick={() => {
