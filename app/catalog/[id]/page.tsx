...
import catalogs from import catalogs from '../../../data/catalogs.json';
...
export default function CatalogPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    setShareUrl(url);
    QRCode.toCanvas(canvasRef.current, url, { width: 160 }, (err) => { if (err) console.error(err); });
  }, [item]);
  if (!item) { return <main><p>Catalog inexistent.</p><Link href="/">Înapoi</Link></main> }
  return (
    <main>
